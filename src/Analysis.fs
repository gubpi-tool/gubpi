...
let private analysePathFragment (p: PathFragment) config =
    ...
    if config.method = Boxes || List.exists (fun (x: Guard) -> x.Value.TryToIntervalLinearFunction().IsNone) guards then
        ...
        AnalysePathBox.computeBoundsForFragment p config, 0.0
    else
        // If all guards are linear, use the optimized computation instead. In this case, the unexplored volume is 0
        AnalysePathVolume.computeBoundsForFragment p (config.epsilonScore) (config.epsilonVar), 0.0
...
let private staticBoundsOnPathFragmentScore (pf: PathFragment) =
    ...
    List.fold
        (fun s (x: SymbolicValue) -> s * (x.EvalIntervalMap varBounds).ExtractInterval)
        Interval.One
        pf.ScoreValues
...
let computeHistogram (p: ProgramPath) (config: Hyperparameters) : Interval [] * Interval =

    match Evaluation.cleanUpPath p with
    | None ->
        // This path is infeasible, i.e., has probability 0
        let bounds =
            Array.create config.discretization.NumberOfBins Interval.Zero
        bounds, Interval.Zero
    | Some p ->
